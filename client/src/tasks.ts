...
export async function findHealthier(rootPath: string): Promise<string> {
  if (await exists(path.join(rootPath, "node_modules", ".bin", "healthier"))) {
...
interface HealthierTaskDefinition extends vscode.TaskDefinition {}
...
class FolderTaskProvider {
  ...
  public async getTask(): Promise<vscode.Task> {
    ...
    try {
      let command = await findHealthier(rootPath);

      let kind: HealthierTaskDefinition = {
        type: "healthier"
      };
