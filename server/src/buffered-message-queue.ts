...
export default class BufferedMessageQueue {
	...
	private readonly requestHandlers: Map<
		string,
		{ handler: RequestHandler<any, any, any>; versionProvider?: VersionProvider<any> }
	>;
	...
	private processQueue(): void {
		...
		if (Request.is(message)) {
			...
			if (
				...
				requestMessage.documentVersion !== elem.versionProvider(requestMessage.params)
			) {
				requestMessage.reject(new ResponseError(ErrorCodes.RequestCancelled, "Request got cancelled"));
				return;
			}
			...
			if (
				...
				notificationMessage.documentVersion !== elem.versionProvider(notificationMessage.params)
			) {
				return;
			}
