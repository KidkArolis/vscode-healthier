...
namespace ValidateNotification {
	export const type = new NotificationType<TextDocument, void>("healthier/validate");
}
...
class Linter {
	...
	constructor() {
		...
		this.messageQueue.registerRequest(DocumentFormattingRequest.type, params => {
			...
			return this.connection.workspace.getConfiguration("healthier").then(config => {
		...
		this.connection.onDidChangeConfiguration(params => {
			...
			this.options = settings.healthier ? settings.healthier.options || {} : {};
	...
	private resolveModule(): Thenable<InitializeResult | ResponseError<InitializeError>> {
		...
		return Files.resolveModule(this.workspaceRoot, "healthier").then(
			(healthier: any) => {
				if (!healthier.lintTextSync) {
					return new ResponseError(99, "The healthier package doesn't export a lintTextSync method.", {
						retry: false
					});
				}
			...
			() => {
				if (this.package.isDependency("healthier")) {
					throw new ResponseError<InitializeError>(
						99,
						"Failed to load healthier package. Make sure healthier is installed in your workspace folder using 'npm install healthier' and then press Retry.",
						{ retry: true }
					);
				}
				return result;
	...
	private validate(document: TextDocument): Thenable<void> {
		if (!this.package.isDependency("healthier")) {
			// Do not validate if `healthier` is not a dependency
		...
		return this.resolveModule().then(() => {
			...
			const report = this.lib.lintTextSync(contents, options);
	...
	private computeAllFixes(uri: string): AllFixesResult | null {
		...
		function createTextEdit(editInfo: AutoFix): TextEdit {
			return TextEdit.replace(
				Range.create(textDocument.positionAt(editInfo.edit.range[0]), textDocument.positionAt(editInfo.edit.range[1])),
				editInfo.edit.text || ""
			);
		}
